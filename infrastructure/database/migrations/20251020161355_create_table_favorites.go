...
func upCreateTableFavorites(ctx context.Context, tx *sql.Tx) error {
	_, err := tx.Exec(`create table favorites("user" uuid not null, "movie" uuid not null, createdAt timestamp, updatedAt timestamp, foreign key("user") references users(id), foreign key("movie") references movies(id), primary key("user", "movie"))`)
	return err
}
